...
\begin{abstract}
Traditional clinical diagnosis relies on analyzing both medical images and electronic health records simultaneously. However, the classical Machine Learning models are incapable to handle multimodal data simultaneously. Moreover, due to strict privacy regulations like HIPAA, GDPR centralizing patient data across institutions is prohibited. While comprehensive diagnosis inherently requires both visual and textual context, securely integrating these modalities across distributed healthcare networks remains a critical challenge. We present OmniMed-FL, a multimodal federated learning framework for multi-class clinical condition classification (Normal, Pneumonia, COVID-19, Pleural Effusion, Cardiomegaly). The framework benchmarks five LLM text encoders, five Vision Transformer (ViT) image encoders, and eight VLM fusion strategies under FedAvg with non-IID Dirichlet(alpha=1.0) partitioning across 5 hospital clients. An anti-collapse stack combining balanced batch sampling, entropy diversity regularization, and adaptive early abort maintains full five-class prediction diversity throughout training. DistilBERT achieves an F1 score of 0.934 on text; ViT-Base achieves an F1 score of 0.664 on images; Concat VLM fusion achieves an F1 score of 0.956, a +2.3\% gain over the best unimodal baseline. Under strict non-IID conditions, federated training retains 99.1\% of centralized accuracy: Fed-VLM matches its centralized counterpart, demonstrating that strong multimodal performance is achievable without sharing patient data. A FAISS-indexed RAG pipeline provides context-grounded retrieval with cosine similarity exceeding 0.89 across all conditions.
...
% framework for multi-class clinical condition classification.
% %(Normal, Pneumonia, COVID-19, Pleural Effusion, Cardiomegaly).
% The framework benchmarks five LLM text encoders, five Vision Transformer (ViT) image encoders, and eight VLM fusion strategies under FedAvg with non-IID Dirichlet($\alpha{=}1.0$) partitioning across $K{=}5$ hospital clients. An anti-collapse stack combining balanced batch sampling, entropy diversity regularization, and adaptive early abort maintains full five-class prediction diversity throughout training. DistilBERT achieves F1\,=\,0.934 on text; ViT-Base achieves
...
%MedFederate addresses this gap. 
The \textit{contributions} of our work is as follows:
\begin{itemize}
  \item A systematic benchmark of 18 model variants (5 LLM, 5 ViT, 8 VLM fusion) for multi-class clinical condition classification under identical non-IID federated conditions. Concat VLM achieves
  ...
  \item A sample balanced clinical image--text corpus using
...
% \begin{figure}[t]
% \centering
% \includegraphics[width=0.95\columnwidth]{plot11_model_heatmap}
...
\begin{tabular}{p{1.8cm} p{5.2cm} c}
\toprule
\textbf{Modality} & \textbf{Model Variants (Macro F1)} & \textbf{Peak F1} \\
\midrule
\textbf{LLM (Text)} & BERT-tiny (0.73), RoBERTa (0.87), ALBERT (0.88), MobileBERT (0.89), DistilBERT (0.93) & 0.93 \\
\midrule
\textbf{ViT (Image)} & ConvNeXT (0.32), EfficientNet (0.36), DeiT (0.65), Swin (0.66), ViT-Base/16 (0.66) & 0.66 \\
\midrule
\textbf{VLM (Fusion)} & CoCa (0.66), CLIP (0.93), Flamingo (0.94), BLIP-2 (0.94), Cross-Attn (0.95), Unified-IO (0.95), Gated (0.96), Concat (0.96) & \textbf{0.96} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.85\linewidth]{plot09_diversity_curves}
...
% \end{subfigure}
% \caption{RAG pipeline evaluation: query correctness (top) and score distribution (bottom).}
% \label{fig:rag_eval}
% \end{figure}
...
\caption{Summary of best-performing models per modality. MedFederate achieves state-of-the-art results for federated multimodal clinical diagnosis.}
