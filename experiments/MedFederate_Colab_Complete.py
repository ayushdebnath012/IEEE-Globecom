...
def get_text_tokenizer(model_name: str = None, max_length: int = 128,
    ...
    """Load the matching tokenizer, failing closed unless fallback is explicit."""
    if not model_name:
    ...
    try:
        ...
        if not allow_fallback:
            raise RuntimeError(
                f"Required tokenizer {tokenizer_name!r} failed to load; refusing "
                "to substitute SimpleTokenizer"
            ) from e
        print(f"  Tokenizer unavailable for {tokenizer_name} ({e}); using explicit fallback")
...
class BalancedBatchSampler:
    def __init__(self, labels: List, batch_size: int = 16,
        ...
        self.present_classes = [i for i, idxs in self.class_indices.items() if idxs]
        n_present = max(len(self.present_classes), 1)
        self.samples_per_class = max(1, batch_size // n_present)
        self.remainder = batch_size - self.samples_per_class * n_present
        # Match an ordinary DataLoader's optimizer-step budget. Basing this on
        # the largest class silently gives highly skewed clients many more local
        # steps and confounds sampler ablations with extra compute.
        n = len(self.flat_labels)
        self.num_batches = max(1, (n // batch_size if drop_last
    ...
    def __iter__(self):
        ...
        for _ in range(self.num_batches):
            batch = []
            for ci in self.present_classes:
                idxs = shuffled[ci]
                for _ in range(self.samples_per_class):
                    p = ptrs[ci] % len(idxs); batch.append(idxs[p]); ptrs[ci] += 1
            if self.remainder > 0:
                sizes = sorted([(i, len(self.class_indices[i])) for i in self.present_classes],
                               key=lambda x: x[1])
                for ci, _ in sizes[:self.remainder]:
                    batch.append(random.choice(shuffled[ci]))
    ...
    def __init__(self, model_name: str = 'prajjwal1/bert-tiny', num_labels: int = 5,
        ...
        if use_pretrained:
            try:
                ...
                raise RuntimeError(
                    f"Required pretrained text backbone {model_name!r} failed to load; "
...
class LightweightVisionClassifier(nn.Module):
    ...
    def __init__(self, model_name: str = 'google/efficientnet-b0', num_labels: int = 5,
        ...
        if use_pretrained:
            try:
                ...
                self.proj = nn.Linear(raw_dim, hidden_dim) if raw_dim != hidden_dim else nn.Identity()
                # The residual CNN exists only as an emergency forward fallback.
                # When the pretrained backbone loads, these parameters are not on
                # the active path and must not be optimized or communicated.
                for fallback in (self.conv1, self.conv2, self.conv3, self.conv4):
                    for p in fallback.parameters():
                        p.requires_grad = False
            except Exception as e:
                raise RuntimeError(
                    f"Required pretrained vision backbone {model_name!r} failed to load; "
                    "refusing to substitute a different architecture"
                ) from e
    ...
    def forward(self, pixel_values, **kwargs):
        if self.backbone is not None:
            # Runtime failures must propagate. In particular, an OOM from the
            # requested ViT must never change the model to the residual CNN in
...
class MultiModalClassifier(nn.Module):
    ...
    def __init__(self, text_model_name: str = 'prajjwal1/bert-tiny',
        ...
        # The unimodal classifiers are bypassed: fusion consumes only each
        # encoder's hidden representation. Exclude these dead heads from local
        # optimization and FedAvg payload accounting.
